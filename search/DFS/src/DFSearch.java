...
public class DFSearch {
    ...
    public static void main(String args[]){
        ...
        if (!dfSearch.depthFirstSearch(getNode(startNode),getNode(startNode),getNode(goalNode),new HashMap()))
            System.out.println(goalNode + " not found");
        else{
    ...
    public boolean depthFirstSearch(int startNode,int node,int goal,HashMap nodesAccessed){
        if (node == goal)
            return true;
        ...
        for (int i=0;i<connections[node].length && !result;i++){
            ...
            if (connections[node][i] == 1 && nodesAccessed.get(getNode(i)) == null) {
                stepsTaken++;
                result = depthFirstSearch(startNode, i, goal, nodesAccessed);
            }
        }
