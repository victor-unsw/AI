...
public class AStarSearch {
    ...
    private int[][] heuristics = new int[20][20];

    private void injectHeuristics(){
        heuristics[ARAD][BUCHAREST] = 366;
        heuristics[BUCHAREST][BUCHAREST] = 0;
        heuristics[CRAIOVA][BUCHAREST] = 160;
        heuristics[DROBETA][BUCHAREST] = 242;
        heuristics[EFORIE][BUCHAREST] = 161;
        heuristics[FAGARAS][BUCHAREST] = 176;
        heuristics[GIURGIU][BUCHAREST] = 77;
        heuristics[HIRSOVA][BUCHAREST] = 151;
        heuristics[IASI][BUCHAREST] = 226;
        heuristics[LUGOJ][BUCHAREST] = 244;
        heuristics[MEHADIA][BUCHAREST] = 241;
        heuristics[NEAMT][BUCHAREST] = 234;
        heuristics[ORADEA][BUCHAREST] = 380;
        heuristics[PITESTI][BUCHAREST] = 100;
        heuristics[RIMNICU][BUCHAREST] = 193;
        heuristics[SIBIU][BUCHAREST] = 253;
        heuristics[TIMISOARA][BUCHAREST] = 329;
        heuristics[URZICENI][BUCHAREST] = 80;
        heuristics[VASLUI][BUCHAREST] = 199;
        heuristics[ZERIND][BUCHAREST] = 374;
    }
    ...
    public static void main(String args[]){
        ...
        AStarSearch gbfs = new AStarSearch();
        gbfs.makeConnections();
        gbfs.injectHeuristics();
        ...
        Position<DoublyLinkedList<Integer>> path = gbfs.greedyBestFirstSearch(getNode(startNode), getNode(goalNode));
        showPath(path);
    }
    ...
    public static void showPath(Position<DoublyLinkedList<Integer>> position){
        ...
        System.out.println(" ]");
        System.out.println("Path cost : " + position.getPathCost());
    }

    public Position<DoublyLinkedList<Integer>> greedyBestFirstSearch(int start,int goal){
        ...
        path.addFirst(start);
        frontier.enqueue(path, getPathCost(0,start,start,goal),0);
        ...
        while (!frontier.isEmpty()){
            ...
            System.out.println("Expanding : " + getNode(node));
            ...
            for (int i=0;i<connections[node].length;i++){
                ...
                if (connections[node][i] != 0 && exploredNodes.get(getNode(i)) == null && !checkOccurance(i,frontier)){
                    ...
                    newPath.addFirst(i);
                    frontier.enqueue(newPath,getPathCost(position.getPathCost(),node,i,goal),position.getPathCost()+connections[node][i]);
                }else if (connections[node][i] != 0 && exploredNodes.get(getNode(i)) == null && checkHigherOccurance(getPathCost(position.getPathCost(),node,i,goal),i,frontier)){
                    DoublyLinkedList<Integer> newPath = path.getDuplicate();
                    newPath.addFirst(i);
                    frontier.enqueue(newPath,getPathCost(position.getPathCost(),node,i,goal),position.getPathCost()+connections[node][i]);
                }
    ...
    private int getPathCost(int currentPathCost,int parentNode,int currentNode, int goalNode) {
        return heuristics[currentNode][goalNode] + currentPathCost + connections[parentNode][currentNode];
    }
    ...
    private boolean checkHigherOccurance(int priority,int element,Queue<DoublyLinkedList<Integer>> frontier){
        ...
        while (!frontier.isEmpty()){
            ...
            else tempQueue.enqueue(path,position.getPriority(),position.getPathCost());
        }
        while (!frontier.isEmpty()){
            Position<DoublyLinkedList<Integer>> position = frontier.dequeue();
            tempQueue.enqueue(position.getElement(),position.getPriority(),position.getPathCost());
        }
        while (!tempQueue.isEmpty()){
            Position<DoublyLinkedList<Integer>> position = tempQueue.dequeue();
            frontier.enqueue(position.getElement(), position.getPriority(),position.getPathCost());
        }
    ...
    private boolean checkOccurance(int element,Queue<DoublyLinkedList<Integer>> frontier){
        ...
        while (!frontier.isEmpty() && !result){
            ...
            if (path.first() == element)
                result = true;
            tempQueue.enqueue(path,position.getPriority(),position.getPathCost());
        }
        while (!frontier.isEmpty()){
            Position<DoublyLinkedList<Integer>> position = frontier.dequeue();
            tempQueue.enqueue(position.getElement(),position.getPriority(),position.getPathCost());
